 ...
 */
class MassCancelEvent extends \Magento\Backend\App\Action
{
    /**
     * {@inheritdoc}
     */
    ...
    public function execute()
    {
        try {
            ...
            foreach ($collection->getAllIds() as $eventQueueId) {
                if ($this->eventQueueManagement->cancelEvent($eventQueueId)) {
                    $count++;
                }
            }
        ...
        if ($count > 0) {
            $this->messageManager->addSuccessMessage(
                __('A total of %1 email chain(s) have been cancelled.', $count)
            );
        } else {
            $this->messageManager->addErrorMessage(
                __('None of selected email chains can be cancelled.')
            );
        }
