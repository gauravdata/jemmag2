...
class Processor
{
    ...
    public function setOptionsByFilter($attribute, $filter)
    {
        ...
        if ($defaultOptionId) {
            ...
            $attribute->setData('default', $default);
        }
    ...
    protected function getNewOptionsFromFilter($filter)
    {
        ...
        if ($filterExtensionAttributes->getSwatches()) {
            /** @var SwatchInterface[] $swatches */
            $swatches = $filterExtensionAttributes->getSwatches();
            foreach ($swatches as $swatchItem) {
                $option = $this->filterSwatchConverter->toAttributeOption($swatchItem);
                $newOptions[] = $option;
            }
        }
    ...
    protected function getOptionsData($options, $optionsIdsToDelete)
    {
        ...
        foreach ($options as $optionItem) {
            $optionIndex++;
            $optionId = $optionItem->getValue() ?: 'option_' . $optionIndex;
